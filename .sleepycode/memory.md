...
- **GDPR export (A.29): DONE** — `GET /api/users/me/export` (auth) via `UsersExportHandler` + `userDataAggregator` (user, profile, submission_count; password_hash json:"-"). Profile “Download My Data” → `aioj-data-export.json`. Tests: export JSON + 401 without claims. Verified: go build/test, tsc. Next: A.30 account deletion.
