        ...
        <div>
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Configure language mappings for remote OJ platforms</p>
                </div>
            ...
            <div className="flex gap-2 mb-4">
                {PLATFORMS.map(p => (
                    <button key={p.value} onClick={() => setPlatform(p.value)}
                        className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${platform === p.value ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                        {p.label}
                    </button>
                ))}
            </div>
            ...
            {showForm && (
                <div className="bg-white border border-gray-200 rounded-lg p-5 mb-6">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-6 gap-4 items-end">
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Local ID</label>
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Remote ID</label>
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Comment Prefix</label>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={resetForm} className="px-4 py-2 text-sm text-gray-600 hover:text-black">Cancel</button>
                        </div>
                    </form>
                </div>
            )}
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {langs.map(l => (
                                <tr key={l.id} className={!l.enabled ? 'bg-gray-50 opacity-60' : ''}>
                                    ...
                                    <td className="px-4 py-3">
                                        <button onClick={() => handleToggle(l)}
                                            className={`px-2 py-0.5 rounded text-xs font-medium cursor-pointer ${l.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                            {l.enabled ? 'Enabled' : 'Disabled'}
                                        </button>
                                    </td>
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => startEdit(l)} className="text-blue-600 hover:text-blue-800"><Pencil className="w-4 h-4" /></button>
                                            <button onClick={() => handleDelete(l.id)} className="text-red-600 hover:text-red-800"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                            {langs.length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">No language mappings configured</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
