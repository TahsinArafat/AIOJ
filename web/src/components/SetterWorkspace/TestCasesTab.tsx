...
export default function TestCasesTab({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 border-b pb-2 mb-4">Upload Testcase Package (ZIP)</h2>
      ...
      <div>
        <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 border-b pb-2 mb-4">Testcase Scores & Breakdown</h2>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {testcases.map((tc, idx) => (
                <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-800 dark:text-gray-200">{tc.input_name}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-800 dark:text-gray-200">{tc.output_name}</td>
                  <td className="px-4 py-2.5 text-gray-800 dark:text-gray-200">{tc.score} pts</td>
        ...
        {testcases.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 mb-4 flex items-end justify-between">
            <div className="flex gap-4 items-end">
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Set Score for All Test Cases</label>
                <input
                  type="number"
                  value={batchScore}
                  onChange={e => onBatchScoreChange(Number(e.target.value))}
                  className="border border-gray-300 dark:border-gray-600 rounded px-3 py-1 text-xs w-28 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200"
                />
              </div>
              <button
                onClick={onBatchSet}
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 space-y-4">
          <h4 className="font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Register TestCase File Matches</h4>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Input Name</label>
              <input
                type="text"
                placeholder="e.g. 01.in"
                value={newTestCase.input_name}
                onChange={e => onNewTestCaseChange({ ...newTestCase, input_name: e.target.value })}
                className="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200"
              />
            </div>
            <div>
              <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Output Name</label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200"
              />
            </div>
          </div>
          <button
