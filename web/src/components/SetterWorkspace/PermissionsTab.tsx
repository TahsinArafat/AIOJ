...
export default function PermissionsTab({
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 border-b pb-2 mb-4">Problem Collaborators</h2>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {collaborators.map((c, idx) => (
              <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-4 py-2.5 font-medium text-gray-800 dark:text-gray-200">{c.username}</td>
                <td className="px-4 py-2.5">
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold uppercase tracking-wider ${
                    ...
                    c.access_level === 'co_author' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200'
                  }`}>
                    {c.access_level}
                  </span>
                </td>
                <td className="px-4 py-2.5 text-right">
                  {c.access_level !== 'owner' ? (
                    ...
                    <span className="text-gray-400 dark:text-gray-500 text-xs font-medium">Primary Owner</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
        <h4 className="font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Add Collaborator</h4>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 text-xs bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
            >
